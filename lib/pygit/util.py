...
class RemoteProgress(object):
	"""
	Handler providing an interface to parse progress information emitted by pygit-push
	and pygit-fetch and to dispatch callbacks allowing subclasses to react to the progress.
	"""
	_num_op_codes = 5
	...
	def _parse_progress_line(self, line):
		"""Parse progress information from the given line as retrieved by pygit-push
		or pygit-fetch
		...
		for sline in sub_lines:
			...
			# them as they are non-ascii. As pygit might expect a tty, it will send them
...
class Actor(object):
	...
	mentioned in the pygit log entries."""
	# PRECOMPILED REGEX
	...
	def _from_string(cls, string):
		...
		:param string: is the string, which is expected to be in regular pygit format
	...
	def committer(cls, config_reader=None):
		...
		:return: Actor instance corresponding to the configured committer. It behaves
			similar to the pygit implementation, such that the environment will override 
...
class Stats(object):
	"""
	Represents stat information as presented by pygit at the end of a merge. It is 
	...
	def _list_from_string(cls, repo, text):
		"""Create a Stat object from output retrieved by pygit-diff.
...
class Iterable(object):
	...
	way to retrieve and iterate items within the pygit repository"""
	__slots__ = tuple()
