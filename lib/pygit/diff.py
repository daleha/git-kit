...
class Diffable(object):
	...
	def _process_diff_args(self, args):
		...
		:return:
			...
			Method is called right before pygit command execution.
	...
	def diff(self, other=Index, paths=None, create_patch=False, **kwargs):
		...
		:param kwargs:
			Additional arguments passed to pygit-diff, such as 
			R=True to swap both sides of the diff.

		:return: pygit.DiffIndex
			
		:note:
			Rename detection will only work if create_patch is True.
			
			On a bare repository, 'other' needs to be provided as Index or as 
			as Tree/Commit, or a pygit command error will occour"""
		args = list()
