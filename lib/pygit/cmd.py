...
# Enables debugging of GitPyPython's pygit commands
...
class GitPy(object):
	...
	# CONFIGURATION
	# The size in bytes read from stdout when copying pygit's output to another stream
	...
	def execute(self, command,
		...
		:param with_exceptions:
			Whether to raise an exception when pygit returns a non-zero status.
		...
		:param output_stream:
			If set to a file-like object, data produced by the pygit command will be 
	...
	def transform_kwargs(self, **kwargs):
		"""Transforms Python style kwargs into pygit command line options."""
		args = list()
	...
	def _call_process(self, method, *args, **kwargs):
		"""Run the given pygit command with the specified arguments and return
		...
		:param args:
			...
			This allows your commands to call pygit more conveniently as None
	...
	def _parse_object_header(self, header_line):
		...
		if len(tokens) != 3:
			if not tokens:
				raise ValueError("SHA could not be resolved, pygit returned: %r" % (header_line.strip()))
			else:
				raise ValueError("SHA %s could not be resolved, pygit returned: %r" % (tokens[0], header_line.strip()))
