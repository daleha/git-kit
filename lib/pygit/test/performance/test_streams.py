...
class TestObjDBPerformance(TestBigRepoR):
	...
	def test_large_data_streaming(self, rwrepo):
		...
		for randomize in range(2):
			...
			# del db file so pygit has something to do
			os.remove(db_file)
			...
			del(data)
			assert pygitsha == bin_to_hex(binsha)		# we do it the same way, right ?
			
			#  as its the same sha, we reuse our path
			fsize_kib = os.path.getsize(db_file) / 1000
			print >> sys.stderr, "Added %i KiB (filesize = %i KiB) of %s data to using pygit-hash-object in %f s ( %f Write KiB / s)" % (size_kib, fsize_kib, desc, gelapsed_add, size_kib / gelapsed_add)
			
			# compare ... 
			print >> sys.stderr, "GitPy-Python is %f %% faster than pygit when adding big %s files" % (100.0 - (elapsed_add / gelapsed_add) * 100, desc)
			...
			print >> sys.stderr, "Read %i KiB of %s data at once using pygit-cat-file in %f s ( %f Read KiB / s)" % (size_kib, desc, gelapsed_readall, size_kib / gelapsed_readall)

			# compare 
			print >> sys.stderr, "GitPy-Python is %f %% faster than pygit when reading big %sfiles" % (100.0 - (elapsed_readall / gelapsed_readall) * 100, desc)
			...
			print >> sys.stderr, "Read %i KiB of %s data in %i KiB chunks from pygit-cat-file in %f s ( %f Read KiB / s)" % (size_kib, desc, cs_kib, gelapsed_readchunks, size_kib / gelapsed_readchunks)
			
			# compare 
			print >> sys.stderr, "GitPy-Python is %f %% faster than pygit when reading big %s files in chunks" % (100.0 - (elapsed_readchunks / gelapsed_readchunks) * 100, desc)
