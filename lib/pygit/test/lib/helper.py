...
import os
import sys
from pygit import Repo, Remote, GitPyCommandError
...
def with_rw_and_rw_remote_repo(working_tree_ref):
	...
	rw_repo has been forked as well as a handle for a pygit-daemon that may be started to 
	run the remote_repo.
	...
	def argument_passer(func):
		def remote_repo_creator(self):
			...
			try:
				...
				print str(e)
				if os.name == 'nt':
					raise AssertionError('git-daemon needs to run this test, but windows does not have one. Otherwise, run: pygit-daemon "%s"'%tempfile.gettempdir()) 
				else:
					raise AssertionError('Please start a pygit-daemon to run this test, execute: git-daemon "%s"'%tempfile.gettempdir())
...
class TestBase(TestCase):
	...
	- Class level repository which is considered read-only as it is shared among 
	  ...
	  The rorepo is in fact your current project's pygit repo. If you refer to specific 
