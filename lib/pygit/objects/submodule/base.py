...
from pygit.objects.util import Traversable
...
from pygit.config import SectionConstraint
...
class Submodule(util.IndexObject, Iterable, Traversable):
	"""Implements access to a pygit submodule. They are special in that their sha
	...
	def _set_cache_(self, attr):
		if attr == '_parent_commit':
			...
			# pygit-python extension values - optional
			self._branch_path = reader.get_value(self.k_head_option, pygit.Head.to_full_path(self.k_head_default))
		elif attr == '_name':
	...
	def remove(self, module=True, force=False, configuration=True, dry_run=False):
		...
		if configuration and not dry_run:
			...
			# now pygit config - need the config intact, otherwise we can't query 
			# inforamtion anymore
	...
	def module_exists(self):
		""":return: True if our module exists and is a valid pygit repository. See module() method"""
		try:
			self.module()
			return True
		except Exception:
	...
	def branch_name(self):
		...
		return pygit.Head(self.repo, self._branch_path).name
	...
	def name(self):
		...
		:note: by default, the name is the path at which to find the submodule, but
			in pygit-python it should be a unique identifier similar to the identifiers
	...
	def iter_items(cls, repo, parent_commit='HEAD'):
		...
		for sms in parser.sections():
			...
			sm._branch_path = pygit.Head.to_full_path(b)
			sm._url = u
