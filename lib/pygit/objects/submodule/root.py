...
from pygit.exc import InvalidGitPyRepositoryError
import pygit
...
class RootModule(Submodule):
	...
	def __init__(self, repo):
		...
		super(RootModule, self).__init__(
										...
										url = '',
										branch_path = pygit.Head.to_full_path(self.k_head_default)
										)
			...
			if sm.module_exists():
				...
				if sm.branch_path != psm.branch_path:
					...
					if not dry_run:
						...
						smmr = smm.remotes
						try:
							tbr = pygit.Head.create(smm, sm.branch_name, logmsg='branch: Created from HEAD')
						except OSError:
							# ... or reuse the existing one
							tbr = pygit.Head(smm, sm.branch_path)
