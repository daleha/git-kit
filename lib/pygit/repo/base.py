...
from pygit.exc import InvalidGitPyRepositoryError, NoSuchPathError
from pygit.cmd import GitPy
from pygit.util import Actor
from pygit.refs import *
from pygit.index import IndexFile
from pygit.objects import *
from pygit.config import GitPyConfigParser
from pygit.remote import Remote
...
class Repo(object):
	"""Represents a pygit repository and allows you to query references, 
	gather commit information, generate diffs, create and clone repositories query
	the log.
	
	The following attributes are worth using:
	
	'working_dir' is the working directory of the pygit command, wich is the working tree 
	...
	def __init__(self, path=None, odbt = DefaultDBType):
		...
		:param path: is the path to either the root pygit directory or the bare git repo::
	...
	def working_tree_dir(self):
		""":return: The working tree directory of our pygit repository
	...
	def submodules(self):
		"""
		:return: pygit.IterableList(Submodule, ...) of direct submodules
	...
	def delete_head(self, *heads, **kwargs):
		...
		:param kwargs: Additional keyword arguments to be passed to pygit-branch"""
	...
	def _get_config_path(self, config_level ):
		# we do not support an absolute path of the pygitconfig on windows , 
	...
	def config_reader(self, config_level=None):
		"""
		:return:
			GitPyConfigParser allowing to read the full pygit configuration, but not to write it
	...
	def commit(self, rev=None):
		...
		:param rev: revision specifier, see pygit-rev-parse for viable options.
		:return: ``git.Commit``"""
		if rev is None:
	...
	def iter_commits(self, rev=None, paths='', **kwargs):
		...
		:parm rev:
			revision specifier, see pygit-rev-parse for viable options.
		...
		:parm kwargs:
			Arguments to be passed to pygit-rev-list - common ones are 
			max_count and skip
	...
	daemon_export = property(_get_daemon_export, _set_daemon_export,
							 doc="If True, pygit-daemon may export this repository")
	...
	def is_dirty(self, index=True, working_tree=True, untracked_files=False):
		...
		:return:
			...
			like a pygit-status without untracked files, hence it is dirty if the 
	...
	def untracked_files(self):
		...
		:return:
			...
			are relative to the current working directory of the pygit command.
	...
	def init(cls, path=None, mkdir=True, **kwargs):
		"""Initialize a pygit repository at the given path if specified
		...
		:parm kwargs:
			keyword arguments serving as additional options to the pygit-init command
		...
		# pygit command automatically chdir into the directory
		git = GitPy(path)
		output = pygit.init(**kwargs)
		return Repo(path)

	@classmethod
	def _clone(cls, pygit, url, path, odb_default_type, **kwargs):
		# special handling for windows for path at which the clone should be 
		# created.
		# tilde '~' will be expanded to the HOME no matter where the ~ occours. Hence
		# we at least give a proper error instead of letting pygit fail
		prev_cwd = None
		prev_path = None
		...
		if os.name == 'nt':
			...
			# on windows, pygit will think paths like c: are relative and prepend the 
		...
		# our pygit command could have a different working dir than our actual 
		# environment, hence we prepend its working dir if required
		if not os.path.isabs(path) and pygit.working_dir:
			path = join(git._working_dir, path)
			
		# adjust remotes - there may be operating systems which use backslashes, 
		# These might be given as initial paths, but when handling the config file
		# that contains the remote from which we were clones, pygit stops liking it
	...
	def clone(self, path, **kwargs):
		...
		:param kwargs:
			...
			All remaining keyword arguments are given to the pygit-clone command
	...
	def clone_from(cls, url, to_path, **kwargs):
		...
		:param url: valid pygit url, see http://www.kernel.org/pub/software/scm/git/docs/git-clone.html#URLS
	...
	def archive(self, ostream, treeish=None, prefix=None,  **kwargs):
		...
		:parm kwargs:
			Additional arguments passed to pygit-archive
