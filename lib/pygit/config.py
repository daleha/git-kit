...
from pygit.odict import OrderedDict
from pygit.util import LockFile
...
class GitPyConfigParser(cp.RawConfigParser, object):
	"""Implements specifics required to read pygit style configuration files.
	
	This variation behaves much like the pygit.config command such that the configuration
	...
	def _read(self, fp, fpname):
		...
		Made sure it ignores initial whitespace as pygit uses tabs"""
	...
	def get_value(self, section, option, default = None):
		...
		# try boolean values as pygit uses them
		vl = valuestr.lower() 
		if vl == 'false':
			return False
		if vl == 'true':
