...
def post_clear_cache(func):
	"""Decorator for functions that alter the index using the pygit command. This would
...
def default_index(func):
	...
	repository index. This is as we rely on pygit commands that operate
	on that index only. """
	def check_default_index(self, *args, **kwargs):
		if self._file_path != self._index_path():
			raise AssertionError( "Cannot call %r on indices that do not represent the default pygit index" % func.__name__ )
...
def pygit_working_dir(func):
	"""Decorator which changes the current working dir to the one of the pygit 
