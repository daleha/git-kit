...
import pygit.objects
import pygit.diff as diff
...
from pygit.objects.util import Serializable
...
class IndexFile(LazyMixin, diff.Diffable, Serializable):
	...
	order to save pygit command function calls wherever possible.
	...
	before operating on it using the pygit command"""
	...
	def new(cls, repo, *tree_sha):
		...
		This method behaves like pygit-read-tree --aggressive when doing the merge.
	...
	def _iter_expand_paths(self, paths):
		...
		Note: pygit will add items multiple times even if a glob overlapped
	...
	def _write_path_to_stdin(self, proc, filepath, item, fmakeexc, fprogress, 
		...
		:note: There is a bug in pygit-update-index that prevents it from sending
		...
		:note: Newlines are essential here, pygits behaviour is somewhat inconsistent
	...
	def _to_relative_path(self, path):
		""":return: Version of path relative to our pygit directory or raise ValueError
		if it is not within our pygit direcotory"""
		if not os.path.isabs(path):
			return path
		relative_path = path.replace(self.repo.working_tree_dir+os.sep, "")
		if relative_path == path:
			raise ValueError("Absolute path %r is not in pygit repository at %r" % (path,self.repo.working_tree_dir))
	...
	def remove(self, items, working_tree=False, **kwargs):
		...
		:param items:
			...
			- path string
				...
				to a path relative to the pygit repository directory containing
				the working tree
		...
		:param kwargs:
			Additional keyword arguments to be passed to pygit-rm, such
	...
	def move(self, items, skip_errors=False, **kwargs):
		...
		:param skip_errors:
			...
			be skpped.
		:param kwargs:
			Additional arguments you would like to pass to pygit-mv, such as dry_run
			or force.
		...
		:raise ValueErorr: If only one item was given
			GitPyCommandError: If pygit could not handle your request"""
		args = list()
		if skip_errors:
			args.append('-k')
	...
	def reset(self, commit='HEAD', working_tree=False, paths=None, head=False, **kwargs):
		...
		:param kwargs:
			Additional keyword arguments passed to pygit-reset
		...
		.. note:: IndexFile.reset, as opposed to HEAD.reset, will not delete anyfiles
			...
			If you want pygit-reset like behaviour, use *HEAD.reset* instead.

		:return: self """
		# what we actually want to do is to merge the tree into our existing
		# index, which is what pygit-read-tree does
	...
	def diff(self, other=diff.Diffable.Index, paths=None, create_patch=False, **kwargs):
		...
		:note:
			Will only work with indices that represent the default pygit index as
