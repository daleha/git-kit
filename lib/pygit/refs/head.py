...
class HEAD(SymbolicReference):
	...
	def reset(self, commit='HEAD', index=True, working_tree = False, 
		...
		:param paths:
			Single path or list of paths relative to the pygit root directory
			that are to be reset. This allows to partially reset individual files.
		
		:param kwargs:
			Additional arguments passed to pygit-reset. 
		...
		if index:
			...
			# it appears, some pygit-versions declare mixed and paths deprecated
		...
		try:
			...
			# pygit nowadays may use 1 as status to indicate there are still unstaged
...
class Head(Reference):
	...
	def rename(self, new_path, force=False):
		...
		:return: self
		:note: respects the ref log as pygit commands are used"""
		flag = "-m"
		if force:
			flag = "-M"
	...
	def checkout(self, force=False, **kwargs):
		...
		:param kwargs:
			Additional keyword arguments to be passed to pygit checkout, i.e.
